...
CCommonItemBone::CCommonItemBone(_vec3 vOriginPos)
	: CDiveItem(vOriginPos)
{
}

CCommonItemBone::CCommonItemBone(const CCommonItemBone& rhs)
	: CDiveItem(rhs)
{
}

CCommonItemBone::~CCommonItemBone()
{
}

HRESULT CCommonItemBone::Ready_GameObject()
{
	...
	D3DXIMAGE_INFO imgInfo = *static_cast<CAssetTexture*>(CAssetMgr::GetInstance()->Get_Asset(L"Tex_UIITem_Bone")->at(0))->Get_ImgInfo();
	imgInfo.Width;
	m_wsTexName = L"Tex_UIITem_Bone";
...
_int CCommonItemBone::Update_GameObject(const _float& fTimeDelta)
{
...
void CCommonItemBone::LateUpdate_GameObject(const _float& fTimeDelta)
{
	if (m_eCurState == ITEMSTATE::ACQUIRED)
		return;
...
void CCommonItemBone::Render_GameObject()
{
	LPDIRECT3DDEVICE9 pGraphicDev = CGraphicDev::GetInstance()->Get_GraphicDev();

	pGraphicDev->SetTransform(D3DTS_WORLD, m_pTransformCom->Get_World());

	if (auto vecAsset = CAssetMgr::GetInstance()->Get_Asset(L"Tex_UIITem_Bone"))
	{
...
void CCommonItemBone::GetItem()
{
	...
	if (auto pLayer = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"2_Fish_Layer"))
	{
		...
		pGetItemUI->Set_ImgAssetName(L"Tex_UIITem_Bone");
		pGetItemUI->Set_Title(L"정체불명의 뼈");
		pGetItemUI->Set_Rank(L"Rank_");
		pGetItemUI->Set_Weight(L"1.5kg");
		...
		//fish.iRank = m_iRank;
		//fish.iStar = m_iStar;
		fish.sFishName = L"정체불명의 뼈";
		fish.sThumbNailAssetName = L"Tex_UIITem_Bone";
...
void CCommonItemBone::OpenItemDesc()
{
	if (auto pUI = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"0_UI_Layer")->Get_GameObjectFirst<CDiveItemDescUI>(L"DiveItemDescUI"))
	{
		if (!pUI->Get_Render())
		{
			pUI->Set_Title(L"정체불명의 뼈");
			pUI->Set_Desc(L"누군가의 뼈다. 사람의 것인지 명확히 알 수 없다.");
			pUI->Set_Render(true);
		}
	}
}

HRESULT CCommonItemBone::Ready_Component()
{
	// 버퍼
...
CCommonItemBone* CCommonItemBone::Create(_vec3 vOriginPos)
{
	CCommonItemBone* pItem = new CCommonItemBone(vOriginPos);
	if (FAILED(pItem->Ready_GameObject()))
	{
		Safe_Release(pItem);
		MSG_BOX("CCommonItemBone Create Failed");
		return nullptr;
	}
