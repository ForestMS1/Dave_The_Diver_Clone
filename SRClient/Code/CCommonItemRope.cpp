...
CCommonItemRope::CCommonItemRope(_vec3 vOriginPos)
	: CDiveItem(vOriginPos)
{
}

CCommonItemRope::CCommonItemRope(const CCommonItemRope& rhs)
	: CDiveItem(rhs)
{
}

CCommonItemRope::~CCommonItemRope()
{
}

HRESULT CCommonItemRope::Ready_GameObject()
{
...
_int CCommonItemRope::Update_GameObject(const _float& fTimeDelta)
{
...
void CCommonItemRope::LateUpdate_GameObject(const _float& fTimeDelta)
{
	if (m_eCurState == ITEMSTATE::ACQUIRED)
		return;
...
void CCommonItemRope::Render_GameObject()
{
...
void CCommonItemRope::GetItem()
{
	...
	if (auto pLayer = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"2_Fish_Layer"))
	{
		...
		pGetItemUI->Set_Title(L"밧줄");
		pGetItemUI->Set_Rank(L"Rank_");
		pGetItemUI->Set_Weight(L"0.69kg");
...
void CCommonItemRope::OpenItemDesc()
{
...
HRESULT CCommonItemRope::Ready_Component()
{
	// 버퍼
...
CCommonItemRope* CCommonItemRope::Create(_vec3 vOriginPos)
{
	CCommonItemRope* pItem = new CCommonItemRope(vOriginPos);
	if (FAILED(pItem->Ready_GameObject()))
	{
		Safe_Release(pItem);
		MSG_BOX("CCommonItemRope Create Failed");
		return nullptr;
	}
