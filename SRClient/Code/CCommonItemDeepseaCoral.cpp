...
CCommonItemDeepseaCoral::CCommonItemDeepseaCoral(_vec3 vOriginPos)
	: CDiveItem(vOriginPos)
{
}

CCommonItemDeepseaCoral::CCommonItemDeepseaCoral(const CCommonItemDeepseaCoral& rhs)
	: CDiveItem(rhs)
{
}

CCommonItemDeepseaCoral::~CCommonItemDeepseaCoral()
{
}

HRESULT CCommonItemDeepseaCoral::Ready_GameObject()
{
	...
	m_wsTexName = L"Tex_UIITem_DeepseaCoral";
...
_int CCommonItemDeepseaCoral::Update_GameObject(const _float& fTimeDelta)
{
...
void CCommonItemDeepseaCoral::LateUpdate_GameObject(const _float& fTimeDelta)
{
	if (m_eCurState == ITEMSTATE::ACQUIRED)
		return;
...
void CCommonItemDeepseaCoral::Render_GameObject()
{
...
void CCommonItemDeepseaCoral::GetItem()
{
	CDiveItem::GetItem();
	...
	if (auto pLayer = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"2_Fish_Layer"))
	{
		...
		pGetItemUI->Set_Title(L"심해 산호");
		pGetItemUI->Set_Rank(L"Rank_");
		pGetItemUI->Set_Weight(L"0.6kg");
...
void CCommonItemDeepseaCoral::OpenItemDesc()
{
	if (auto pUI = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"0_UI_Layer")->Get_GameObjectFirst<CDiveItemDescUI>(L"DiveItemDescUI"))
	{
		if (!pUI->Get_Render())
		{
			pUI->Set_Title(L"심해 산호");
			pUI->Set_Desc(L"깊은 바다 속 산호.");
			pUI->Set_Render(true);
		}
	}
}

HRESULT CCommonItemDeepseaCoral::Ready_Component()
{
	// 버퍼
...
CCommonItemDeepseaCoral* CCommonItemDeepseaCoral::Create(_vec3 vOriginPos)
{
	CCommonItemDeepseaCoral* pItem = new CCommonItemDeepseaCoral(vOriginPos);
	if (FAILED(pItem->Ready_GameObject()))
	{
		Safe_Release(pItem);
		MSG_BOX("CCommonItemDeepseaCoral Create Failed");
		return nullptr;
	}
...
void CCommonItemDeepseaCoral::Free()
{
	CDiveItem::Free();
	CGameObject::Free();
}
