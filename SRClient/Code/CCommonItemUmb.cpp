...
CCommonItemUmb::CCommonItemUmb(_vec3 vOriginPos)
	: CDiveItem(vOriginPos)
{
}

CCommonItemUmb::CCommonItemUmb(const CCommonItemUmb& rhs)
	: CDiveItem(rhs)
{
}

CCommonItemUmb::~CCommonItemUmb()
{
}

HRESULT CCommonItemUmb::Ready_GameObject()
{
	...
	m_wsTexName = L"Tex_UIITem_Umbellula";
...
_int CCommonItemUmb::Update_GameObject(const _float& fTimeDelta)
{
...
void CCommonItemUmb::LateUpdate_GameObject(const _float& fTimeDelta)
{
	if (m_eCurState == ITEMSTATE::ACQUIRED)
		return;
...
void CCommonItemUmb::Render_GameObject()
{
...
void CCommonItemUmb::GetItem()
{
	...
	if (auto pLayer = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"2_Fish_Layer"))
	{
		...
		pGetItemUI->Set_Title(L"움벨 룰라");
		pGetItemUI->Set_Rank(L"Rank_");
		pGetItemUI->Set_Weight(L"2.73kg");
...
void CCommonItemUmb::OpenItemDesc()
{
	if (auto pUI = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"0_UI_Layer")->Get_GameObjectFirst<CDiveItemDescUI>(L"DiveItemDescUI"))
	{
		if (!pUI->Get_Render())
		{
			pUI->Set_Title(L"움벨 룰라");
			pUI->Set_Desc(L"깊은 바다에 서식하는 산호충아문 바다조랭이류의 한 속.");
			pUI->Set_Render(true);
		}
	}
}

HRESULT CCommonItemUmb::Ready_Component()
{
	// 버퍼
...
CCommonItemUmb* CCommonItemUmb::Create(_vec3 vOriginPos)
{
	CCommonItemUmb* pItem = new CCommonItemUmb(vOriginPos);
	if (FAILED(pItem->Ready_GameObject()))
	{
		Safe_Release(pItem);
		MSG_BOX("CCommonItemUmb Create Failed");
		return nullptr;
	}
