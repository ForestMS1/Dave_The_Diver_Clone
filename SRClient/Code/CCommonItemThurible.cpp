...
CCommonItemThurible::CCommonItemThurible(_vec3 vOriginPos)
	: CDiveItem(vOriginPos)
{
}

CCommonItemThurible::CCommonItemThurible(const CCommonItemThurible& rhs)
	: CDiveItem(rhs)
{
}

CCommonItemThurible::~CCommonItemThurible()
{
}

HRESULT CCommonItemThurible::Ready_GameObject()
{
	...
	m_wsTexName = L"Tex_UIITem_JadeThurible";
...
_int CCommonItemThurible::Update_GameObject(const _float& fTimeDelta)
{
...
void CCommonItemThurible::LateUpdate_GameObject(const _float& fTimeDelta)
{
	if (m_eCurState == ITEMSTATE::ACQUIRED)
		return;
...
void CCommonItemThurible::Render_GameObject()
{
...
void CCommonItemThurible::GetItem()
{
	CDiveItem::GetItem();
	...
	if (auto pLayer = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"2_Fish_Layer"))
	{
		...
		pGetItemUI->Set_Title(L"향로");
		pGetItemUI->Set_Rank(L"Rank_");
		pGetItemUI->Set_Weight(L"1.78kg");
		...
		fish.bFish = false;
		fish.sItemDesc = L"향로의 향을 맡고 있다보면\n매우 기분이 좋아진다.";
...
void CCommonItemThurible::OpenItemDesc()
{
	if (auto pUI = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"0_UI_Layer")->Get_GameObjectFirst<CDiveItemDescUI>(L"DiveItemDescUI"))
	{
		...
		{
			pUI->Set_Title(L"향로");
			pUI->Set_Desc(L"향로의 향을 맡고 있다보면 매우 기분이 좋아진다.");
			pUI->Set_Render(true);
		}
	}
}

HRESULT CCommonItemThurible::Ready_Component()
{
	// 버퍼
...
CCommonItemThurible* CCommonItemThurible::Create(_vec3 vOriginPos)
{
	CCommonItemThurible* pItem = new CCommonItemThurible(vOriginPos);
	if (FAILED(pItem->Ready_GameObject()))
	{
		Safe_Release(pItem);
		MSG_BOX("CCommonItemThurible Create Failed");
		return nullptr;
	}
