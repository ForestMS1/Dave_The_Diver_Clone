...
CLogoTitle::CLogoTitle(float fPosX, float fPosY)
    : CGameObject()
    , m_fPosX(fPosX)
    , m_fPosY(fPosY)
{
...
HRESULT		CLogoTitle::Ready_GameObject()
{
    ...
    float fAs = 1474 / 656.f;
    _vec3 vScale = { fAs * 0.3f , 1.f * 0.3f, 1.f };
...
void		CLogoTitle::Render_GameObject()
{
    ...
    if (auto vecAsset = CAssetMgr::GetInstance()->Get_Asset(L"Tex_Logo_Title"))
    {
        if (auto pTexture = dynamic_cast<CAssetTexture*>(vecAsset->at(0)))
        {
...
HRESULT			CLogoTitle::Ready_Component()
{
    // 버퍼
...
CLogoTitle* CLogoTitle::Create(float fPosX, float fPosY)
{
    CLogoTitle* pTitle = new CLogoTitle{ fPosX , fPosY };

    if (FAILED(pTitle->Ready_GameObject()))
    {
        Safe_Release(pTitle);
        MSG_BOX("pTitle Create Failed");
        return nullptr;
    }
