...
CCommonItemWoodPlate::CCommonItemWoodPlate(_vec3 vOriginPos)
	: CDiveItem(vOriginPos)
{
}

CCommonItemWoodPlate::CCommonItemWoodPlate(const CCommonItemWoodPlate& rhs)
	: CDiveItem(rhs)
{
}

CCommonItemWoodPlate::~CCommonItemWoodPlate()
{
}

HRESULT CCommonItemWoodPlate::Ready_GameObject()
{
	...
	D3DXIMAGE_INFO imgInfo = *static_cast<CAssetTexture*>(CAssetMgr::GetInstance()->Get_Asset(L"Tex_UIITem_Woodplate")->at(0))->Get_ImgInfo();
	imgInfo.Width;
	m_wsTexName = L"Tex_UIITem_Woodplate";
...
_int CCommonItemWoodPlate::Update_GameObject(const _float& fTimeDelta)
{
...
void CCommonItemWoodPlate::LateUpdate_GameObject(const _float& fTimeDelta)
{
	if (m_eCurState == ITEMSTATE::ACQUIRED)
		return;
...
void CCommonItemWoodPlate::Render_GameObject()
{
	LPDIRECT3DDEVICE9 pGraphicDev = CGraphicDev::GetInstance()->Get_GraphicDev();

	pGraphicDev->SetTransform(D3DTS_WORLD, m_pTransformCom->Get_World());

	if (auto vecAsset = CAssetMgr::GetInstance()->Get_Asset(L"Tex_UIITem_Woodplate"))
	{
...
void CCommonItemWoodPlate::GetItem()
{
	CDiveItem::GetItem();
	...
	if (auto pLayer = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"2_Fish_Layer"))
	{
		...
		pGetItemUI->Set_ImgAssetName(L"Tex_UIITem_Woodplate");
		pGetItemUI->Set_Title(L"나무판");
		...
		//fish.iRank = m_iRank;
		//fish.iStar = m_iStar;
		fish.sFishName = L"나무판";
		fish.sThumbNailAssetName = L"Tex_UIITem_Woodplate";
...
void CCommonItemWoodPlate::OpenItemDesc()
{
	if (auto pUI = CManagement::GetInstance()->Get_Scene()->Get_Layer(L"0_UI_Layer")->Get_GameObjectFirst<CDiveItemDescUI>(L"DiveItemDescUI"))
	{
		...
		{
			pUI->Set_Title(L"나무판");
			pUI->Set_Desc(L"나무판이다.");
			pUI->Set_Render(true);
		}
	}
}

HRESULT CCommonItemWoodPlate::Ready_Component()
{
	// 버퍼
...
CCommonItemWoodPlate* CCommonItemWoodPlate::Create(_vec3 vOriginPos)
{
	CCommonItemWoodPlate* pItem = new CCommonItemWoodPlate(vOriginPos);
	if (FAILED(pItem->Ready_GameObject()))
	{
		Safe_Release(pItem);
		MSG_BOX("CCommonItemWoodPlate Create Failed");
		return nullptr;
	}
...
void CCommonItemWoodPlate::Free()
{
	CDiveItem::Free();
	CGameObject::Free();
}
