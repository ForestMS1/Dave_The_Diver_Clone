...
COverlay::COverlay(const CGameObject& rhs)
    : CGameObject(rhs)
{
}
...
HRESULT COverlay::Ready_GameObject()
{
...
_int COverlay::Update_GameObject(const _float& fTimeDelta)
{
    if (m_bRender) {
...
void COverlay::LateUpdate_GameObject(const _float& fTimeDelta)
{
    if (m_bRender) {
...
void COverlay::Render_GameObject()
{
    if (m_bRender) {
        ...
        m_pOverlayTextureCom->Set_Texture(0);
...
HRESULT COverlay::Ready_Component()
{
    // 버퍼
    ...
    // 텍스쳐
    if (FAILED((AddComponent<Engine::CTexture, ID_STATIC>(L"Proto_OverlayTex", L"Com_Texture", &m_pOverlayTextureCom))))
        return E_FAIL;
    // 트랜스폼
