#include "CCubeCol.h"
#include "CGraphicDev.h"

CCubeCol::CCubeCol()
{
}

CCubeCol::CCubeCol(const CCubeCol& rhs)
	: CVIBuffer(rhs)
{
}

CCubeCol::~CCubeCol()
{
}

HRESULT CCubeCol::Ready_Buffer(_ulong dwColor)
{

	m_dwVtxSize = sizeof(VTXCUBECOL);
	m_dwVtxCnt = 8;
	m_dwTriCnt = 12;
	m_dwFVF = FVF_CUBECOL;

	m_dwIdxSize = sizeof(INDEX32);
	m_IdxFmt = D3DFMT_INDEX32;

	if (FAILED(CVIBuffer::Ready_Buffer()))
		return E_FAIL;

	VTXCUBECOL* pVertex = NULL;

	// &pVertex : 버텍스버퍼에 보관된 버텍스 중 첫 번째 버텍스의 주소를 얻어옴

	m_pVB->Lock(0, 0, (void**)&pVertex, 0);

	pVertex[0].vPosition = { -1.f, 1.f, -1.f };
	pVertex[0].dwColor = dwColor;

	pVertex[1].vPosition = { 1.f, 1.f, -1.f };
	pVertex[1].dwColor = dwColor;

	pVertex[2].vPosition = { 1.f, -1.f, -1.f };
	pVertex[2].dwColor = dwColor;

	pVertex[3].vPosition = { -1.f, -1.f, -1.f };
	pVertex[3].dwColor = dwColor;

	pVertex[4].vPosition = { -1.f, 1.f, 1.f };
	pVertex[4].dwColor = dwColor;

	pVertex[5].vPosition = { 1.f, 1.f, 1.f };
	pVertex[5].dwColor = dwColor;

	pVertex[6].vPosition = { 1.f, -1.f, 1.f };
	pVertex[6].dwColor = dwColor;

	pVertex[7].vPosition = { -1.f, -1.f, 1.f };
	pVertex[7].dwColor = dwColor;

	m_pVB->Unlock();

	INDEX32* pIndex = nullptr;

	m_pIB->Lock(0, 0, (void**)&pIndex, 0);

	// X+
	pIndex[0]._0 = 1;
	pIndex[0]._1 = 5;
	pIndex[0]._2 = 6;

	// X+
	pIndex[1]._0 = 1;
	pIndex[1]._1 = 6;
	pIndex[1]._2 = 2;

	// X-
	pIndex[2]._0 = 4;
	pIndex[2]._1 = 0;
	pIndex[2]._2 = 3;

	// X-
	pIndex[3]._0 = 4;
	pIndex[3]._1 = 3;
	pIndex[3]._2 = 7;

	// Y+
	pIndex[4]._0 = 4;
	pIndex[4]._1 = 5;
	pIndex[4]._2 = 1;

	// Y+
	pIndex[5]._0 = 4;
	pIndex[5]._1 = 1;
	pIndex[5]._2 = 0;

	// Y-
	pIndex[6]._0 = 3;
	pIndex[6]._1 = 2;
	pIndex[6]._2 = 6;

	// Y-
	pIndex[7]._0 = 3;
	pIndex[7]._1 = 6;
	pIndex[7]._2 = 7;

	// Z+
	pIndex[8]._0 = 7;
	pIndex[8]._1 = 6;
	pIndex[8]._2 = 5;

	// Z+
	pIndex[9]._0 = 7;
	pIndex[9]._1 = 5;
	pIndex[9]._2 = 4;

	// Z-
	pIndex[10]._0 = 0;
	pIndex[10]._1 = 1;
	pIndex[10]._2 = 2;

	// Z-
	pIndex[11]._0 = 0;
	pIndex[11]._1 = 2;
	pIndex[11]._2 = 3;

	m_pIB->Unlock();

	return S_OK;
}

void CCubeCol::Render_Buffer()
{
	CVIBuffer::Render_Buffer();
}

CCubeCol* CCubeCol::Create(_ulong dwColor)
{
	CCubeCol* pCubeTex = new CCubeCol;

	if (FAILED(pCubeTex->Ready_Buffer(dwColor)))
	{
		Safe_Release(pCubeTex);
		MSG_BOX("pCubeTex Create Failed");
		return nullptr;
	}

	return pCubeTex;
}

CComponent* CCubeCol::Clone()
{
	return new CCubeCol(*this);
}

void CCubeCol::Free()
{
	CVIBuffer::Free();
}
