...
void CRenderer::Render_SushiBGAlpha(LPDIRECT3DDEVICE9& pGraphicDev)
{
	pGraphicDev->SetRenderState(D3DRS_ZWRITEENABLE, FALSE);
	...
	m_RenderGroup[RENDER_SUSHIBGALPHA].sort([](CGameObject* pDst, CGameObject* pSrc)->bool
		{
			return pDst->Get_ViewZ() > pSrc->Get_ViewZ();
		});

	for (auto& pObj : m_RenderGroup[RENDER_SUSHIBGALPHA])
		pObj->Render_GameObject();
...
void CRenderer::Render_AlphaAfterOrtho(LPDIRECT3DDEVICE9& pGraphicDev)
{
	pGraphicDev->SetRenderState(D3DRS_ZWRITEENABLE, FALSE);
	...
	m_RenderGroup[RENDER_ALPHA_AFTER_ORTHO_UI].sort([](CGameObject* pDst, CGameObject* pSrc)->bool
		{
			return pDst->Get_ViewZ() > pSrc->Get_ViewZ();
		});

	for (auto& pObj : m_RenderGroup[RENDER_ALPHA_AFTER_ORTHO_UI])
		pObj->Render_GameObject();
...
void CRenderer::Render_Transition(LPDIRECT3DDEVICE9& pGraphicDev)
{
	pGraphicDev->SetRenderState(D3DRS_ZWRITEENABLE, FALSE);
	...
	m_RenderGroup[RENDER_TRANSITION].sort([](CGameObject* pDst, CGameObject* pSrc)->bool
		{
			return pDst->Get_ViewZ() > pSrc->Get_ViewZ();
		});

	for (auto& pObj : m_RenderGroup[RENDER_TRANSITION])
		pObj->Render_GameObject();
